...
**Solved:** Changed CSS from fixed positioning to absolute positioning, and made getDropdownPosition return a constant position
...
**Solved:** Added htmlFor={inputId} to label for the checkbox
...
**Solved:** In the onChange function for InputSelect as defined in App.tsx, first check if the id is an empty string, and if so load all transactions. Otherwise, load transactions as normal.
...
**Solved:** In usePaginatedTransactions.ts, in setPaginatedTransactions concatenated new response data to old response data
...
**Solved:** In loadAllTransactions, moved setIsLoading(false) before starting paginatedTransactions request
...
**Solved:** Check if paginatedTransactions has a nextPage before rendering the button
...
**Solved:** Endpoint cache is cleared every time approval data changes
